 ...
 */
public class TestHybridtoProlepticConversion {
  @Test
  public void testReadWrite() throws Exception {
    Configuration conf = new Configuration();
    // convert to proleptic calendar
    conf.set("orc.proleptic.gregorian", "true");
    Reader reader = OrcFile.createReader(new Path("/home/chong/下载/read-hybrid-as-proleptic.orc"),
    ...
    while (rowIterator.nextBatch(batch)) {
      for (int row = 0; row < batch.size; ++row) {
        ...
        System.out.println("c1: " + (x.noNulls || !x.isNull[xRow] ? x.vector[xRow] : null));
      }
    }
    rowIterator.close();
  }
